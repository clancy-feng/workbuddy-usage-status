...
A：credit 来自 `session_usage`，是会话级汇总值，源数据没有按天拆分。抽取器把整个会话的 credit 归因到它「首次出现」的那天，所以跨越多天的大任务会全部算在首日。这是当前数据源的粒度极限（见已知限制第 1 条）。
...
A：用自然语言描述「查看 / 生成 WorkBuddy 使用状态」即可，无需记关键词。说得太模糊可能识别不到，换种更明确的说法就行（见第 3 节入口 A）。
