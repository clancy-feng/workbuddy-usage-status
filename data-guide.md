...
- **会话归属** `session_id` = trace 的 `sessionId`；约 **44%** 的 trace 携带它，其余（多为 `auto` / 旧格式）无法归因到「具体会话」。注意：这个 44% 只影响「按会话 / 按模型」的**下钻归因**，不影响总量（见下方「已知口径限制」第 2 条）。
...
2. **归因覆盖率约 44% —— 不代表报告不可靠**：这里"覆盖率"指的是**能把一条请求精确归到某个会话**的比例（即 trace 携带 `sessionId`）。它**只影响"按会话/按模型"的下钻视图**（Top 10 会话、模型性价比排行），这部分只对已归因的 ~44% 生效，未归因的归入 `unknown` / `auto`，所以会话级数字会**偏少、不是全量**。
   - **报告的整体结论依然可靠**：总 Credit 来自 `workbuddy.db` 的 `session_usage` 表（与 trace 是否带 `sessionId` 无关），总 Token 来自全部 trace 的实际用量（同样不依赖 `sessionId`）。因此**总量、每日趋势、模型占比、错误数这些"宏观"指标是完整且准确的**；偏少的只是"某个会话 / 某个模型具体吃了多少"这种"微观"归因。
   - 简言之：看全局用量看板是准的；想精确到"哪次会话最费"，要意识到它只覆盖约 44% 的会话，剩下在 `unknown` 里。
