...
### 4.9 错误明细（分类统计，基于全量数据）

「错误明细」卡片与上方日期筛选**无关**——它统计的是**全量** trace 数据，不受起止日期影响，因此能看到完整历史里的错误分布，而不是当前筛选窗口内的。看板默认只展示「高频错误内容」一张表（最常用），其余 5 类（类型 / 工具 / 模型 / 会话 / 近期样本）集中在「导出 CSV」里，避免卡片过长。

早期版本只把错误记成「错误数」一个总数，丢弃了每条 span 的具体错误信息。现在的卡片把这些信息**还原为可下钻的分类统计**：
...
这些分类共同回答"错误主要是什么、由什么工具/模型/会话引发、最近是否还在发生"，是错误监控的主入口。该卡片还提供「导出 CSV」按钮，把上述 6 类统计合并导出为单个 CSV（带 UTF-8 BOM，Excel 直接打开中文不乱码；无错误数据时按钮自动禁用）。
